...
function eliminar(codigo_oficina) {
    Swal.fire({
        ...
        if (result.value) {
            $.post('../../controllers/oficina.controllers.php?op=eliminar', {
...
var cargaTabla = () => {
    var html = '';
    var Usuario_IdRoles = document.getElementById('DetalleRoll').value;
    $.post('../../controllers/oficina.controllers.php?op=todos', (datos) => {
        ...
        $.each(datos, (index, val) => {
            html += '<tr><td>' + (
                index + 1
            ) + '</td><td>' + val.ciudad + '</td><td>' + val.pais + '</td><td>' + val.region + '</td><td>' + val.codigo_postal + '</td><td>' + val.telefono + '</td><td>' + val.linea_direccion1 + '</td><td>' + val.linea_direccion2 + '</td>';
            if ((Usuario_IdRoles) === "Administrador") {
                html += '<td><button type=button; onClick=editar(' + val.codigo_oficina + ') class=btn&#32;btn-outline-success>Editar</button>'
                html += '<button type=button onClick=eliminar(' + val.codigo_oficina + ') class=btn&#32;btn-outline-danger&#32;btn-icon>Eliminar</button></td></tr>';

            }
            else {
                html += '<td><button type=button onClick=editar(' + val.codigo_oficina + ') class=btn&#32;btn-outline-success>Editar</button></tr>';
            }
        });
